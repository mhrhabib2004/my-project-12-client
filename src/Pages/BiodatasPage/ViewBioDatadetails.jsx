...
const ViewBioDatadetails = () => {
    ...
    if(loading){
     ...
     const {biodataType,name,profileImage,dateOfBirth,height,weight,age,occupation,race,fathersName,mothersName,permanentDivision,presentDivision,expectedPartnerAge,expectedPartnerHeight,expectedPartnerWeight,contactEmail,mobileNumber,biodataId,_id}=bio;
    ...
    return (
        <div>
            ...
            <div className="py-5 bg-slate-300 text-center">
                ...
                {user?.isPremium ? (
            <>
              <h1>Contact Email: {contactEmail}</h1>
              <h1>Mobile Number: {mobileNumber}</h1>
                ...
                <Link to={`/addfavourits/${_id}`} className="px-5 py-2.5 relative rounded group overflow-hidden font-medium bg-purple-50 text-purple-600 inline-block">
                    <span className="absolute top-0 left-0 flex w-full h-0 mb-0 transition-all duration-200 ease-out transform translate-y-0 bg-purple-600 group-hover:h-full opacity-90"></span>
                    <span className="relative group-hover:text-white">Add favourits</span>
                </Link>
                {!user?.isPremium && (
              <button onClick={handleRequestContact} className="px-5 py-2.5 relative rounded group overflow-hidden font-medium bg-purple-50 text-purple-600 inline-block">
                <span className="absolute top-0 left-0 flex w-full h-0 mb-0 transition-all duration-200 ease-out transform translate-y-0 bg-purple-600 group-hover:h-full opacity-90"></span>
